...
\begin{titlepage}
    ...
    \begin{center}
        ...
        \HRule \\[1.5cm]
        \textsc{\LARGE Issambre L'Hermite Dumont \\ Simon Desmidt}\\[3cm]
...
\begin{equation}
	\max \left( \frac{t_{arith}}{o},\frac{t_{mem}}{o} \right)
\end{equation}
With some algebra, we can find the \textbf{number of operations per second}:
\begin{equation}
	\min \left( \text{CPU freq}, a \times \text{bandwidth} \right)
\end{equation}
To resume:
\begin{itemize}
...
\end{equation}
\textcolor{red}{TODO: FIX TYPO FROM SLIDE}
...
SIMD (Single Instruction Multiple Data) is a parallel computing architecture used to process multiple data simultaneously with a single instruction.
\section{How does it work?}
SIMD takes advantages of \textbf{vector registers}, to perform the same operation on multiple data elements in parallel within a single CPU cycle. It uses an extension of a CPU architecture that features specific register and set of instructions. So it stores the vector on a specific register and uses the optimized instructions on it. It increases the throughput of processed data.\\
SIMD uses sets of instructions and registers to increases the performances, for example AVX512 is an extension of a CPU architecture that features a specific set instructions designed to work with wider registers than AVX2 . Possible extensions are: SSE, AVX, AVX2, AVX512, NEON, etc.\\
...
\end{lstlisting}
Here we use \texttt{int} which is $32$ bits so if we use SIMD with AVX2, we can store $(256/32) = 8$ integers in an AVX2 register. And so SIMD, with that type of set of instructions, will automatically translate the previous code to:
\begin{lstlisting}[style=CppStyle]
int a[] = {...};
int b[] = {...};
int c[size];
...
\end{lstlisting}
We can write the code with the intrinsics (all the function with \_ before), for manual control, but it's not necessary. The compiler will do it for you. Here we reduce the number of loop iteration by a factor of 8, but we increases the number of instructions.\\
We can thus use SIMD to speed up the code. To use it, we need to ask the compiler for it with flags. Here are some examples (certains flags aren't available for some compiler):\\
\begin{center}
	\begin{tabular}{|c|c|}
		\hline
		\textbf{Purpose} & \textbf{Flags}\\
		\hline
		Optimization level & \texttt{-O2}, \texttt{-O3} \\
		\hline
		Sets of instructions of architectures & \texttt{-mavx}, \texttt{-mavx2}, \texttt{-mavx512f}, etc. \\
		\hline
		Maximum optimization for own CPU& \texttt{-march=native} \\
		\hline
	\end{tabular}
\end{center}
...
\begin{center}
	\begin{tabularx}{\textwidth}{|X|X|}
		...
		\hline
		Speedup of about $8$, for supported operations & Data must be aligned in memory \\
		\hline
		Energetic efficiency because fewer optimized instructions & Not fitted for all algorithm (especially with data dependencies)\\
		\hline
		Better usage of the CPU, especially memory wise & If intrinsics are used, code is less portable\\
		\hline
		Simple to add to an existing code & Order of operation can be changed, possible floating point inaccuracies.\\
		\hline
	\end{tabularx}
\end{center}
...
\end{lstlisting}
\subsection{Reduce}
\textbf{Reduce} sums up all the values of the variables on the different processes and store the sum on one process.\\
\begin{center}
	\begin{tabular}{ccccc|c|ccccc}
		Before &&&&& $\Longrightarrow$ &After&&&&\\
		\hline
