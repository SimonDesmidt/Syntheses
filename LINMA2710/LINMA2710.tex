...
SIMD takes advantages of \textbf{vector registers}, to perform the same operation on multiple data elements in parallel within a single CPU cycle. It uses an extension of a CPU architectures that features specific register and set of instructions. So it stores the vector the specific registrer and uses the optimized instructions on it. It increases the throughput of processed data.\\
SIMD uses sets of instructions and registers to increases the performances, for example AVX512 is an extension of a CPU architecture that features a specific set instructions designed to work with wider registers than AVX2 . Possibles extensions are: SSE, AVX, AVX2, AVX512, NEON, etc.\\
