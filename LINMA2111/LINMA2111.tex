...
\begin{titlepage}
    ...
    \begin{center}
        ...
        \HRule \\[1.5cm]
        \textsc{\LARGE Charles Van Hees \\Simon Desmidt\\ Issambre L'Hermite Dumont}\\[3cm]
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
\section{Searching for a better complexity}
...
\end{proof}
\begin{equation}
    \phi(x_1,x_2,x_3,x_4) = \color{blue}\left(x_2 \vee x_1 \vee \bar x_3\right) \color{black}\bigwedge \color{green}\left(\bar x_1 \vee \bar x_2 \vee x_4\right) \color{black} \bigwedge \color{red} \left(x_2 \vee \bar x_4 \vee  x_3\right)\color{black}
\end{equation}
\begin{center}
    \begin{tikzpicture}[
        ...
        % Horizontal nodes 
        \node[circle, draw=green, ultra thick] (C1) at (4,0)  {$\bar x_1$};
        \node[circle, draw=green] (C2) at (7,0)  {$\bar x_2$};
        \node[circle, draw=green] (C3) at (10,0)  {$x_4$};
        
        % Right nodes
        \node[circle, draw=blue] (A1) at (2,-2) {$x_2$};
        \node[circle, draw=blue] (A2) at (2,-4) {$x_1$};
        \node[circle, draw=blue, ultra thick] (A3) at (2,-6) {$\bar x_3$};
        
        % Left nodes
        \node[circle, draw=red, ultra thick] (B1) at (12,-2) {$x_2$};
        \node[circle, draw=red] (B2) at (12,-4) {$\bar x_4$};
        \node[circle, draw=red] (B3) at (12,-6) {$x_3$};
        ...
        \draw[color=violet, ultra thick] (A3) -- (C1);
        \draw[edge] (A3) -- (C2);
        \draw[edge] (A3) -- (C3);
        \draw[color=violet, ultra thick] (A3) -- (B1);
        \draw[edge] (A3) -- (B2);

        \draw[color=violet, ultra thick] (C1) -- (B1);
        \draw[edge] (C1) -- (B2);
        \draw[edge] (C1) -- (B3);
    ...
    \end{tikzpicture}
\end{center}
The clique is in violet. We put every element of the clique to True, and if some variable does not appear in the clique, its value is irrelevant and can be either True or False. In our example, the solutions are $(0,1,0,1)$ and $(0,1,0,0)$. 
\end{document}