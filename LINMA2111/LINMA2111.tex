...
\begin{itemize}
    \item Insert (entry priority): ine-serts the entry with the given priority;
...
\end{itemize}
The insertion consists in adding the new neode to the last level in the first free position, and swap it with its father until the heap is properly re-established ($\mathcal{O}(\log(n))$). The ExtractMax consists in removing the root (max entry) and putting the last entry at the root, then swap the new root with its largest child until the heap is properly restored. \\
...
\end{equation}
We call $H(P)$ the entropy of the probabilistic distribution. The entropy of any probabilistic distribution over $N$ elements is at most $\log_2(N)$, which is attained for the uniform distribution. If the $N$ elements are $N$ leaves of a binary tree, enconded by their path from root to leaf (giving the prefix code), we obtain the following theorem:
\begin{thm}
    The expected length of a path root to leaf of an $N$-leaf binary tree, with probability $\rho_1,\dots,\rho_N$ on the leaves, is at least the entropy $H(P)$. In particular, if all leaves occur equally likely, the average length of the path is at least $\log_2(N)$. 
\end{thm}
\begin{thm}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \max_{x_i} \E_B[t(A, xi)] &= \max_{x_i}\sum_b t(A,x_i|B=b)Pr[B=b]\\
            & \ge \E_{x_i} \sum_b t(A,x_i|B=b)Pr[B=b]\\
            & \ge \min_b \E_{x_i} t(A, x_i|B=b)Pr[B=b]
        \end{aligned}
    \end{equation}
...
\end{itemize}
To take advantage of both representations, we must choose a list of evaluation points $z_0,\dots,z_{N-1}$ for which the conversion between representations is fast to compute. \\

We can take $z_k = \omega^k$, for $k=0,\dots, N-1$ and $\omega=\exp{2\pi i /N}$, where $\omega$ is an $N$th root of unity, meaning $z_k$ are all the roots of unity, satisfying $z_k^N = 1$. The sequence of evaluations $(A(z_0), \dots, A(z_{N-1}))$ is called the Discrete Fourier Transform (DFT) of the sequence of coefficients $(a_0, \dots,a_{N-1})$, denoted $\mathcal{F}(a_0,\dots,a_{N-1})$. \\
...
\end{equation}
where $\omega^2$ is a $N/2$-th root of unity, by definition of $\omega$. Thus, the sequence of $\{A_{\text{even}}((\omega^k)^2)\}_{k=1}^{\frac{N}{2}-1}$ is the DFT of the even coefficients, $\mathcal{F}_{N/2}(a_0,a_2,\dots,a_{N-2})$. As for $k=\frac{N}{2},\dots, N-1$, we observe that $(\omega^2)^{N/2+\ell}=(\omega^2)^\ell$ since $\omega^N=1$. Therefore, the sequence of $A_{\text{even}}((\omega^k)^2)$ for $k=\frac{N}{2},\dots, N-1$ is once again $\mathcal{F}_{N/2}(a_0,\dots, a_{N-2})$. \\
The same reasoning can be made with $A_{\text{odd}}((\omega^k)^2)$, which creates the sequence $$\{(\mathcal{F}_{N/2}(a_1,a_3,\dots,a_{N-1}), \mathcal{F}_{N/2}(a_1,a_3,\dots,a_{N-1}))\}_{k=0}^{N-1}$$
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
If $N$ is a power of $2$, we can apply this formula recursively, with the following relation for complexity:
\begin{equation}
    t_N = 2 t_{N/2} + \Theta(N) \Longrightarrow t_N = \Theta(N\log(N))
\end{equation}
Finally, let us notice that $\mathcal{F}_N(a) = Va$, where $a$ is the vector of coefficients and $V$ is a Vandermonde matrix such that $V_{jk} = \omega^{jk}$ for $j,k=0,\dots,N-1$. With this, $V^{-1} = \frac{1}{N} V^*$ where the star denotes the conjugate transpose of $V$.\\
...
\begin{enumerate}
    \item Compute the FFT of the coefficients of $A$ and the FFT of the coefficients of $B$, padded with zeros so as to reach $2N-2$ or the next power of 2; [$\Theta(N\log(N))$]
    \item Multiply the two resulting Fourier domain representations pointwise, which results in the Fourier domain representation of the coefficients of $C$; [$\Theta(N)$]
...
\end{equation}
A naive implementation is the direct translation into a recursive algorithm using $\begin{pmatrix}
    n\\0
\end{pmatrix} = \begin{pmatrix}
    n\\n 
...
\end{equation}
\begin{itemize}
    \item Example with the Fibonacci sequence: $F_0=0, F_1=1, F_n = F_{n-1}+F_{n-2}$ for all $n\ge 2$. We define the power series: 
\end{itemize}
\begin{equation}
    f(z) = \sum_{k\ge 0}F_k z^k
\end{equation}
From the recurrence equation, we decuce that 
\begin{equation}
    \begin{aligned}
        \sum_{k\ge0} F_{k+2} z^{k+2} = \sum_{k\ge 0} F_{k+1}z^{k+2} + F_k z^{k+2}\\
        \Longleftrightarrow f(z)-F_0-F_1z = z(f(z)-F_0)+z^2f(z)\\
        \Longleftrightarrow f(z) = \frac{F_0+(F_1-F_0)z}{z^2+z-1} = \frac{z}{(z+\varphi)(z-\bar\varphi)}
    \end{aligned}
\end{equation}
where $\varphi = \frac{1+\sqrt{5}}{2}$ and $\bar \varphi = \frac{1-\sqrt{5}}{2}$. Using partial fraction decomposition, 
\begin{equation}
    \begin{aligned}
        f(z) = \frac{1}{\sqrt{5}} \left(\frac{\bar \varphi}{z+\bar\varphi}-\frac{\varphi}{z+\varphi}\right) = \frac{1}{\sqrt{5}} \sum_{k\ge 0} (\varphi^k-\bar \varphi^k)z^k\\
        \Longrightarrow F_n = \frac{\varphi^n -\bar \varphi^n}{\sqrt{5}}
    \end{aligned}
\end{equation}
\begin{itemize}
    \item Example with the Catalan numbers: $C_0 = C_1 = 1$, $C_{n+1} = \sum_{k=0}^n C_k C_{n-k}$. We define the power series:
\end{itemize}
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
This gives $f(z) = \frac{1-\sqrt{1-4z}}{2z}$, rejecting the positive sign since $f(0)=1$. By the generalized binomial formula, 
\begin{equation}
    \sqrt{1-4z} = \sum_{n=0}^\infty \begin{pmatrix}
        1/2\\n
    \end{pmatrix} (-4z)^n = 1+\sum_{n=1}^\infty \frac{\left(\frac{1}{2}\right)\left(-\frac{1}{2}\right)\dots \left(\frac{3}{2}-n\right)}{n!}(-1)^n 2^{2^n}z
\end{equation}
Which gives 
\begin{equation}
    f(z) = -\frac{1}{2z} \sum_{n=1}^\infty \frac{\left(\frac{1}{2}\right)\left(-\frac{1}{2}\right)\dots \left(\frac{3}{2}-n\right)}{n!}(-1)^n 2^{2^n}z
\end{equation}
...
\begin{thm}
    ...
    \end{equation}
\end{thm}
\begin{thm}\textbf{Generalized binomial formula.}
    \begin{equation}
    ...
    \end{itemize}
\end{thm}
\begin{thm}\textbf{Cauchy product formula.}
...
The tie complexity is $\Theta(n\log(n))+\Theta(n)$, as we first need to sort the activities and then do once through the array.
...
\begin{itemize}
    \item Robin Hood effect: for every deterministic integration method, there are functions for which the method will fail, e.g. if the function is zero at all the integration points. Those methods have a very poor worst-case behabiour, while the choice of random points makes all instances behave equally well.
...
\begin{equation}
    m(y|x) = f(y|x) \alpha(y|x) = \min\left\{\frac{f(y)g(x|y)}{f(x)}, f(y|x)\right\}
\end{equation}
\begin{thm}
...
\begin{proof}
    ...
    \end{equation}
    for a uniform $r_1$ in $S$. Bayes concludes that 
    \begin{equation}
        Pr[P(r_1,r_2,\dots,r_n) = 0] \le \frac{d-j}{|S|} + \frac{j}{|S|} =\frac{d}{|S|}
    \end{equation}
\end{proof}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
We may improve this using amplification of stochastic advantage: repeating $k$ times improves the probability lower bound to $\left(1-\frac{d}{|S|}\right)^d$. \\
...
If we have a two-sided error, amplification of stochastic advantage still works: \\ if $Pr[\text{True}|\text{it is true}] \ge 1/2 + \varepsilon$ and $Pr[\text{False}|\text{it is false}] \ge 1/2 + \varepsilon$. Then, we may proceed to a majority vote after $k$ trials. \\
Let $X_i=1$ if the $i$-th run is correct and 0 otherwise. Then, $\E[X_i] \ge 1/2+\varepsilon$. Let us take the worst case: $\E[X_i] = 1/2+\varepsilon$. The repeated algorithm is rong if $\sum_i X_i < n/2$. But
\begin{equation}
    \begin{cases}
...
\begin{algorithm}[H]
    \caption{BBS}
    \begin{algorithmic}[1]
        \State Select random $k$-bit prime numbers $p$ and $q$ such that $p\equiv q\equiv 3\mod 4$;
        \State Let $N=pq$;
        \State Let $s\in \Z_N^*$;
...
Generating truly i.i.d. random bits is far from obvious. Therefore, the number of random bits required by an algorithm is a costly resource. In some circumstances, it is possibel to derandomise random algorithms, i.e. make them use less random bits, or no random bits at all, while keeping the same or similar guarantees on accuracy and time/space complexity.\\
There are two genereic derandomization techniques:
\begin{itemize}
...
\end{equation}
Hence we only need pairwise independence between the random vits, and not full independence. This allows us to use less bits. Indeed, consider for exmaple the variables $X=Y\oplus Z$, with $X,Y,Z\in \{0,1\}$. If $Y$ and $Z$ are pairwise independent, then so are $X$ and $Y$, and $X$ and $Z$; but $X,Y,Z$ are not jointly independent. \\
...
\begin{thm}
    Every non-deterministic TM has an equivalent determinstic TM.
\end{thm}
\begin{proof}
...
\begin{definition}
    A universal TM is a TM that solves the following problem:
    \begin{itemize}
        \item Input: a description of a TM $M$ and a description of a finite word on initial tape $w$. We write $\langle M, w\rangle$.
        \item Output: If $M$ stops when the initial tape is $w$, outputs the finite content of the tape when $M$ stops, i.e. the output $T(M)$ of $T$ on $w$. If $T$ never stops, undefined. 
    \end{itemize}
...
\end{thm}
\begin{proof}
    By contradiciton, let $H$ be a decider for $A_{TM}$. Then 
    \begin{equation}
...
\end{proof}
\begin{thm}
    A language is \textbf{decidable} iff it is Turing-recognizable and co-Turing recognizable. 
\end{thm}
\begin{proof}
...
\begin{prop}
    Let $t(n)\ge n$. Every $t(n)$ time multitape TM has an equivaelnt $\mathcal{O}(t^2(n))$ time single-tape TM. 
\end{prop}
Let $N$ be a nondeterministic TM that is a decider. The running time pf $N$ is the function $f:\N\to\N$ where $f(n)$ is the maximum number of steps that $N$ uses on any branch of its computation on any input of length $n$.
\begin{prop}
...
\end{thm}
Indeed, it is in NP and SAT is polynomial reducible to 3-SAT, using the Conjuction Normal Form, possibly adding some useless variables. 
\begin{thm}
