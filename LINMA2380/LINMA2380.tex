...
\end{thm}
\begin{corollary}
    If $A\ge0\in \R^{n\times n}$ is primitive and stochastic, then $\lim_{m\times \infty}A^m =  xu^T$, where $x$ is as in Theorem \ref{thm:7.5}.
\end{corollary}
...
\section{Theorems}
\begin{thm}
    For any bounded set of matrices $\Sigma \subseteq\R^{n\times n}$, the values $\check \rho(\Sigma)$ and $\rho(\Sigma)$ are equal.
\end{thm}
\begin{thm}
    For any bounded set of matrices $\Sigma \subseteq\R^{n\times n}$, there exists a left-infinite product $\dots A_2A_1$ that does not converge to zero iff $\rho(\Sigma) \ge 1$.
\end{thm}
\begin{corollary}
    For any bounded set of matrices $\Sigma \subseteq\R^{n\times n}$, the corresponding switched dyanmical system like the one in the Markov chains is stable iff $\rho(\Sigma) < 1$.
\end{corollary}
\begin{corollary}
    For any bounded set of matrices $\Sigma \subseteq\R^{n\times n}$, there is an infinite product of these matrices reaching the joint spectral radius, i.e. there is a sequence of matrices $A_0,A_1,\dots$ in $\Sigma$ such that 
    \begin{equation}
        \lim_{t\rightarrow \infty} \lVert A_t\dots A_1 A_0\rVert^{1/t} = \rho(\Sigma)
    \end{equation}
\end{corollary}
\begin{corollary}
    For any set of matrices $\Sigma \subseteq \R^{n\times n}$, there is an infinite product of these matrices reaching the joint spectral subradius, i.e. there is a sequence of matrices $A_0,A_1,\dots$ in $\Sigma$ such that
    \begin{equation}
        \lim_{t\rightarrow \infty} \lVert A_t\dots A_1 A_0\rVert^{1/t} = \check \rho(\Sigma)
    \end{equation}
\end{corollary}
\chapter{Summary}
\begin{figure}[H]
