...
\begin{equation}\label{eq:EF}
    \begin{aligned}
        &\min c^Tx + \E_\omega[q(\omega)^Ty(\omega)]\\
        &Ax = b\\
        &T(\omega)x + W(\omega)y(\omega) = h(\omega)\\
        &x\ge 0,y\ge 0\\
    \end{aligned}
...
\begin{minipage}{.5\textwidth}
    ...
    \end{equation}
\end{minipage} 
and thus the expected value function is $V(x) = \sum_{\omega = 1}^Np_\omega Q_\omega (x)$, from which we can define the following problem
\begin{equation}
	\begin{aligned}
